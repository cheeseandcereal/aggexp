...
import (
	...
	aggexp "github.com/cheeseandcereal/aggexp/experiments/0043-embedded-lock-emission-filtering/pkg/apis/aggexp"
...
func New(store *metastore.Store, bodies *backend.Store, locker *locking.Locker, replicaID string, broadcasterSize int) *REST {
...
func (r *REST) Create(ctx context.Context, obj runtime.Object, createValidation rest.ValidateObjectFunc, _ *metav1.CreateOptions) (runtime.Object, error) {
	...
	// 0043: acquire the embedded lock BEFORE touching the body. On the
	// Create path the metadata CR does not exist, so Acquire
	// CAS-creates it carrying only the lock (pre-acquire RV is empty).
	// A concurrent creator who wins the create race leaves us a fresh
	// held lock → 409.
	...
	// Step 1: store the body on the shared body CRD so that when the
	// commit's metadata-CR informer event fires every replica's
	// StitchForRef can find the body. The body CR's RV is discarded.
	...
	// Step 2: commit — write body hash + KRM metadata onto the CR and
	// release the lock in ONE CR write. Its host etcd RV becomes the
	// authoritative RV of the stitched object; its informer event
	// drives the watch fan-out (and is the ONLY watcher-visible
	// transition — the acquire write was suppressed by the emission
	// filter).
	...
	if err != nil {
		// Roll back the body so a retry is clean. The lock is already
		// cleared by the (attempted) commit write or stays until lease
		// expiry.
	...
	// broadcaster with the host-RV-stamped object.
	return stitched, nil
}
...
func (r *REST) Update(
	...
	// BEFORE acquiring the lock. Acquiring the lock writes the
	// metadata CR and advances its RV, so an OCC check against the
	// POST-acquire RV would spuriously 409 every conditional write.
	// The client's precondition RV must be compared against this
	// pre-acquire value.
	...
	if clientRV != "" && preRV != "" && clientRV != preRV {
		// Genuinely stale conditional update.
		return nil, false, apierrors.NewConflict(groupResource, name,
			fmt.Errorf("the object has been modified; please apply your changes to the latest version and try again"))
	}

	// Acquire the embedded lock (this bumps the CR's RV — invisible to
	// watchers via the emission filter, and already accounted for by
	// the pre-acquire OCC capture above).
	h, lerr := r.locker.Acquire(ctx, ref)
	if lerr != nil {
	...
	// Write the body first so the metadata-CR MODIFIED (commit) event
	// finds the updated body on every replica. The renewal goroutine
	// keeps the lease fresh if this Put is slow (scenario 3).
	body := backend.BodyFromWidget(w)
	...
	// Commit: body hash + metadata + lock release in ONE CR write.
	...
	// Informer publishes the (single) MODIFIED event with host RV.
