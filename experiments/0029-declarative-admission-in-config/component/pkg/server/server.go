...
import (
	...
	generatedopenapi "github.com/cheeseandcereal/aggexp/experiments/0029-declarative-admission-in-config/component/pkg/generated/openapi"
	...
	"github.com/cheeseandcereal/aggexp/experiments/0029-declarative-admission-in-config/component/pkg/grpcbackend"
	schemebuild "github.com/cheeseandcereal/aggexp/experiments/0029-declarative-admission-in-config/component/pkg/scheme"
)
...
// name.
const dynCanonicalName = "github.com/cheeseandcereal/aggexp/experiments/0029-declarative-admission-in-config/component/pkg/dyn.Object"
const dynListCanonicalName = "github.com/cheeseandcereal/aggexp/experiments/0029-declarative-admission-in-config/component/pkg/dyn.ObjectList"
...
type Options struct {
	*runtimeserver.Options
	...
	// AdmissionConfigPath points at a YAML file carrying the
	// declarative admission rules (CEL validations + JSONPath
	// mutations). Empty means "no middleware admission; fall back
	// to backend RPC admission only". New in 0029.
...
func (o *Options) AddFlags(fs *pflag.FlagSet) {
	...
	fs.StringVar(&o.AdmissionConfigPath, "admission-config", o.AdmissionConfigPath,
		"Path to a YAML file carrying declarative admission rules (CEL validations + JSONPath mutations). "+
			"Evaluated in the middleware before forwarding to the backend. "+
			"Empty = no middleware admission (backend RPC admission, if advertised, still runs).")
}
...
func (o *Options) Run(ctx context.Context) error {
	// Load declarative admission config (0029). Empty path is fine;
	// the engine is optional.
	var admissionEngine *admission.Engine
	if o.AdmissionConfigPath != "" {
		cfg, err := admission.Load(o.AdmissionConfigPath)
		if err != nil {
			return err
		}
		eng, err := admission.NewEngine(cfg.Admission)
		if err != nil {
			return fmt.Errorf("compile admission config: %w", err)
		}
		admissionEngine = eng
		klog.Infof("loaded declarative admission: %d mutations, %d validations from %s",
			len(cfg.Admission.Mutations), len(cfg.Admission.Validations), o.AdmissionConfigPath)
	} else {
		klog.Infof("no --admission-config provided; middleware admission disabled")
	}
	...
	klog.Infof("backend reports: group=%s version=%s resource=%s kind=%s singular=%s namespaced=%v writable=%v ssa=%v mutate=%v validate=%v",
		...
		resp.GetSupportsMutation(), resp.GetSupportsValidation())
	...
	storage := grpcbackend.New(grpcbackend.Descriptor{
		...
		SupportsValidation:      resp.GetSupportsValidation(),
		SupportsMutation:        resp.GetSupportsMutation(),
		UseTypedWrapper:         o.UseTypedWrapper,
		Columns:                 cols,
