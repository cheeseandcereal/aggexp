...
import (
	...
	krmv1 "github.com/cheeseandcereal/aggexp/experiments/0029-declarative-admission-in-config/gen/aggexp/krm/v1"
	"github.com/cheeseandcereal/aggexp/experiments/0029-declarative-admission-in-config/component/pkg/admission"
...
type Descriptor struct {
	...
	// SupportsValidation, when true, makes the REST adapter call
	// Validate on every Create/Update before persisting. A Deny
	// response is turned into an Invalid 422 that includes the
	// backend's reason string verbatim. New in 0020.
	SupportsValidation bool
	// SupportsMutation, when true, makes the REST adapter call
	// Mutate BEFORE Validate on every Create/Update; the returned
	// patched object replaces the caller's object for the rest of
	// the write path. New in 0020.
	...
	// Admission is the declarative (CEL + JSONPath) admission
	// engine. May be nil — if so, only the 0020-style backend
	// Validate/Mutate RPCs run (if supported). New in 0029.
...
func (r *REST) Create(ctx context.Context, obj runtime.Object, createValidation rest.ValidateObjectFunc, opts *metav1.CreateOptions) (runtime.Object, error) {
	...
	// Mutating admission (before validating), standard k8s ordering.
	raw, admitErr := r.admit(ctx, "CREATE", ns, nameOf(obj), raw, nil, optsDryRunCreate(opts))
	if admitErr != nil {
		return nil, admitErr
	}
	// Re-decode the possibly-patched object so the returned
	// runtime.Object reflects the mutation (helpful for tests and
	// for any post-create logic the library might do).
	obj, err = r.objFromJSON(raw)
	if err != nil {
		return nil, err
	}
...
func (r *REST) Update(
	...
	// Admission (mutate then validate). For UPDATE, OldObjectJson
	// carries the current stored object. For a create-via-update
	// path (forceAllowCreate==true with no current), we treat it
	// as a CREATE from admission's point of view so the 0003
	// name-based CREATE policy applies.
	op := "UPDATE"
	var oldRaw []byte
	if current == nil {
		op = "CREATE"
	} else {
		oldRaw, _ = encodeUnstructured(current)
	}
	raw, admitErr := r.admit(ctx, op, ns, name, raw, oldRaw, optsDryRunUpdate(opts))
	if admitErr != nil {
...
// optsDryRunCreate returns true iff CreateOptions.DryRun contains
// metav1.DryRunAll. On a dry-run, admission still runs (so the
// client sees validation failures) but the stored object is not
// created — the library path honors DryRun in the REST machinery
// above us.
func optsDryRunCreate(o *metav1.CreateOptions) bool {
	if o == nil {
		return false
	}
	for _, v := range o.DryRun {
...
func optsDryRunUpdate(o *metav1.UpdateOptions) bool {
	if o == nil {
		return false
	}
	for _, v := range o.DryRun {
...
// admit runs admission in two layers:
//
//  1. Middleware-declared rules (0029): JSONPath mutations then CEL
//     validations. Evaluated locally, no backend round-trip.
//  2. Backend-declared hooks (0020): Mutate then Validate gRPC RPCs.
//
// Mutating-before-validating within each layer; layer 1 fully
// completes (mutate+validate) before layer 2 begins, so a
// middleware-denied request never reaches the backend. This is the
// simplest composition rule and matches the "run cheap rules first"
// intuition from the 0022 thesis.
//
// Warnings from either hook are forwarded to the caller through the
// library's warning.AddWarning mechanism; they appear as
// `Warning: 299 -` HTTP response headers and kubectl prints them as
// yellow "Warning:" lines.
func (r *REST) admit(ctx context.Context, op, ns, name string, raw, oldRaw []byte, dryRun bool) ([]byte, error) {
	// --- Layer 1: middleware-declared (CEL + JSONPath). ---
	if r.desc.Admission != nil {
		mutated, _, err := r.desc.Admission.Mutate(op, raw)
		if err != nil {
			return nil, apierrors.NewInternalError(fmt.Errorf("middleware mutate: %w", err))
		}
		raw = mutated

		failures, err := r.desc.Admission.Validate(op, raw, oldRaw)
		if err != nil {
			return nil, apierrors.NewInternalError(fmt.Errorf("middleware validate: %w", err))
		}
		if len(failures) > 0 {
			// Emit all failures in a single 422 so the user sees
			// everything wrong at once. apierrors.NewInvalid
			// accepts a field.ErrorList; fill one entry per
			// failure, using the configured fieldPath when
			// present.
			errs := make(field.ErrorList, 0, len(failures))
			for _, f := range failures {
				fp := field.NewPath("")
				if f.FieldPath != "" {
					fp = field.NewPath(f.FieldPath)
				}
				errs = append(errs, field.Invalid(fp, name, f.Message))
			}
			return nil, apierrors.NewInvalid(
				schema.GroupKind{Group: r.desc.GroupResource.Group, Kind: r.desc.Kind},
				name, errs,
			)
		}
	}

	// --- Layer 2: backend-declared (0020 gRPC RPCs). ---
	user := userFromCtx(ctx)

	if r.desc.SupportsMutation {
		resp, err := r.client.Mutate(ctx, &krmv1.MutateRequest{
			User:          user,
			Namespace:     ns,
			Name:          name,
			Operation:     op,
		...
		if err != nil {
			return nil, r.translateAdmissionErr(err, name, "mutate")
		}
		if patched := resp.GetPatchedObjectJson(); len(patched) > 0 {
			raw = patched
		}
	...
	if r.desc.SupportsValidation {
		resp, err := r.client.Validate(ctx, &krmv1.ValidateRequest{
			User:          user,
			Namespace:     ns,
			Name:          name,
			Operation:     op,
		...
		if err != nil {
			return nil, r.translateAdmissionErr(err, name, "validate")
		}
		for _, w := range resp.GetWarnings() {
			warning.AddWarning(ctx, "", w)
		}
		if !resp.GetAllowed() {
			...
			if reason == "" {
				reason = "admission denied without a reason"
			}
			// apierrors.NewInvalid maps to HTTP 422 with the
			// reason inside metav1.Status.Message. This is what
			// standard admission-webhook denials look like to
			// kubectl.
			fieldErr := field.Invalid(field.NewPath(""), name, reason)
			return nil, apierrors.NewInvalid(
				schema.GroupKind{Group: r.desc.GroupResource.Group, Kind: r.desc.Kind},
				name,
				fieldpkgerrors{fieldErr},
			)
		}
	}
...
// fieldpkgerrors is a tiny adapter that satisfies the ErrorList
// parameter of apierrors.NewInvalid without pulling the list type's
// entire package into the name.
type fieldpkgerrors = field.ErrorList

func (r *REST) translateAdmissionErr(err error, name, hook string) error {
	// Admission transport errors: fail closed. Return an
	// InternalError with a message the client can see. A production
	// deployment might surface this as 503 instead; lab-grade is
	// fine with 500 here.
	st, ok := grpcstatus.FromError(err)
	if !ok {
		return apierrors.NewInternalError(fmt.Errorf("%s admission: %w", hook, err))
	}
	return apierrors.NewInternalError(fmt.Errorf("%s admission: %s", hook, st.Message()))
}
