...
import (
	...
	aggexpv1 "github.com/cheeseandcereal/aggexp/experiments/0003-custom-authorizer-external-policy/pkg/apis/aggexp/v1"
	"github.com/cheeseandcereal/aggexp/experiments/0003-custom-authorizer-external-policy/pkg/apiserver"
	"github.com/cheeseandcereal/aggexp/experiments/0003-custom-authorizer-external-policy/pkg/authz"
	generatedopenapi "github.com/cheeseandcereal/aggexp/experiments/0003-custom-authorizer-external-policy/pkg/generated/openapi"
	"github.com/cheeseandcereal/aggexp/experiments/0003-custom-authorizer-external-policy/pkg/registry/hello"
)
...
type Options struct {
	...
	// PolicyServiceURL is the external HTTP endpoint consulted by the
	// per-request identity-based authorizer for aggexp.io resources.
	// Empty disables the custom authorizer (falls back to the
	// default delegating chain).
...
func (o *Options) AddFlags(fs *pflag.FlagSet) {
	...
	fs.StringVar(&o.PolicyServiceURL, "policy-service-url", o.PolicyServiceURL,
		"URL of the external identity-based policy service; POST JSON, receive {allow, reason}. Empty disables the custom authorizer.")
...
func (o *Options) Config() (*genericapiserver.RecommendedConfig, error) {
	...
	// Chain the external-policy authorizer BEFORE the library's default
	// union (privileged-groups, /healthz path, delegated SAR). Putting
	// ours first is deliberate: with permissive upstream RBAC, the
	// delegated SAR returns Allow for aggexp.io requests, short-circuiting
	// anything after it. Our authorizer is the intended gate for
	// aggexp.io, so it must run first. For every non-aggexp.io request
	// (including /metrics, system:masters bypass, etc.) it returns
	// NoOpinion, letting the library's chain continue to handle them.
	// Empty URL disables chaining.
	if o.PolicyServiceURL != "" {
		...
		klog.Infof("external-policy authorizer chained; URL=%s group=%s (runs before library chain for aggexp.io)", o.PolicyServiceURL, aggexpv1.GroupName)
	}
