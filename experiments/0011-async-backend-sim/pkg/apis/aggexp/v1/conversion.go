...
// (aggexp.Widget) and external (v1.Widget).
...
func convertV1ToInternal(in *Widget, out *aggexp.Widget) error {
	...
	out.Spec = aggexp.WidgetSpec{DesiredState: in.Spec.DesiredState, Config: copyStringMap(in.Spec.Config)}
	out.Status = aggexp.WidgetStatus{
		Phase:         in.Status.Phase,
		ObservedState: in.Status.ObservedState,
...
func convertInternalToV1(in *aggexp.Widget, out *Widget) error {
	out.TypeMeta = in.TypeMeta
	out.ObjectMeta = in.ObjectMeta
	out.Spec = WidgetSpec{DesiredState: in.Spec.DesiredState, Config: copyStringMap(in.Spec.Config)}
	out.Status = WidgetStatus{
		Phase:         in.Status.Phase,
		ObservedState: in.Status.ObservedState,
		ReadyAt:       in.Status.ReadyAt,
		Message:       in.Status.Message,
