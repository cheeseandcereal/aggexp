...
// byte-identical in this experiment; the conversions are mechanical.
// The library's PATCH / SSA machinery uses the internal hub version,
// so both must be registered with 1:1 funcs even when identical.
...
func convertV1ToInternal(in *Widget, out *aggexp.Widget) error {
	...
	out.Spec = aggexp.WidgetSpec{
		Description: in.Spec.Description,
		Counter:     in.Spec.Counter,
		Tags:        copyStringMap(in.Spec.Tags),
	}
	out.Status = aggexp.WidgetStatus{ObservedCounter: in.Status.ObservedCounter}
	return nil
}
...
func convertInternalToV1(in *aggexp.Widget, out *Widget) error {
	...
	out.Status = WidgetStatus{ObservedCounter: in.Status.ObservedCounter}
	return nil
}
