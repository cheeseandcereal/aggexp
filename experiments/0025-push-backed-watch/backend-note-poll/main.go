// Command note-backend-poll is the polling-variant backend for
// experiment 0025. It implements the runtime/component/proto Backend
// gRPC service for notes.aggexp.io/v1 Notes, but deliberately does
// NOT implement Watch — the Watch RPC returns codes.Unimplemented so
// the middleware falls back to a list-poll loop.
//
// No event generator here; state only changes via kubectl writes.
// (The scenarios drive traffic via kubectl apply/patch so we can
// measure how long the middleware's poll loop takes to see each
// change.)
package main
...
func newBackend() *backend {
	return &backend{items: map[key]*Note{}}
}

// GetSchema ships plain JSON Schema (Track B, from 0023). No GVK
// extension, no ObjectMeta, no List wrapper — the middleware
// synthesizes those. Note that this backend does NOT advertise any
// watch-capable property; the capability probe relies on the Watch
// RPC returning Unimplemented.
func (b *backend) GetSchema(_ context.Context, _ *componentpb.GetSchemaRequest) (*componentpb.GetSchemaResponse, error) {
	plain := map[string]any{
		"type":        "object",
		"description": "A Note served by the POLL variant of 0025.",
...
func (b *backend) Create(_ context.Context, req *componentpb.CreateRequest) (*componentpb.CreateResponse, error) {
	...
	b.items[k] = &n
	raw, _ := json.Marshal(&n)
	log.Printf("create name=%s user=%s fm=%q", n.Metadata.Name, userLabel(req.User), req.FieldManager)
...
func (b *backend) Update(_ context.Context, req *componentpb.UpdateRequest) (*componentpb.UpdateResponse, error) {
	...
	b.items[k] = &n
	raw, _ := json.Marshal(&n)
	log.Printf("update name=%s fm=%q", req.Name, req.FieldManager)
...
func (b *backend) Delete(_ context.Context, req *componentpb.DeleteRequest) (*componentpb.DeleteResponse, error) {
	...
	log.Printf("delete name=%s", req.Name)
	return &componentpb.DeleteResponse{ObjectJson: raw, Deleted: true}, nil
}

// Watch deliberately returns Unimplemented. The component's
// capability probe picks this up and switches to poll mode.
func (b *backend) Watch(_ *componentpb.WatchRequest, _ grpc.ServerStreamingServer[componentpb.WatchEvent]) error {
	return status.Error(codes.Unimplemented, "this backend does not support Watch; middleware should poll")
}

// eventGenerator mirrors the push-variant's schedule exactly so the
// two variants can be compared apples-to-apples. Mutations happen in
// the backend's own state map out-of-band (no kubectl write path);
// the middleware only learns about them on its next poll tick.
func (b *backend) eventGenerator(ctx context.Context, namespace string) {
	...
	for {
		...
		b.mu.Unlock()
		log.Printf("gen t=%s op=CREATE name=%s body=v1 cycle=%d",
			time.Now().UTC().Format(time.RFC3339Nano), name, cycle)
		...
		if cur, ok := b.items[key{namespace, name}]; ok {
			...
			log.Printf("gen t=%s op=UPDATE name=%s body=v2 cycle=%d",
				time.Now().UTC().Format(time.RFC3339Nano), name, cycle)
		}
		b.mu.Unlock()
		...
		if cur, ok := b.items[key{namespace, name}]; ok {
			cur.Spec.Body = "v3"
			cur.Status.UpdatedAt = time.Now().UTC().Format(time.RFC3339)
			log.Printf("gen t=%s op=UPDATE name=%s body=v3 cycle=%d",
				time.Now().UTC().Format(time.RFC3339Nano), name, cycle)
		}
		b.mu.Unlock()
		...
		b.mu.Lock()
		if _, ok := b.items[key{namespace, name}]; ok {
			delete(b.items, key{namespace, name})
			log.Printf("gen t=%s op=DELETE name=%s cycle=%d",
				time.Now().UTC().Format(time.RFC3339Nano), name, cycle)
		}
		b.mu.Unlock()
...
func main() {
	...
	enableGen := flag.Bool("enable-event-generator", false, "Run the internal event generator on a fixed schedule (matching the push variant).")
	...
	log.Printf("note-backend-poll (0025 variant A) listening on %s; Watch RPC returns Unimplemented", *addr)
