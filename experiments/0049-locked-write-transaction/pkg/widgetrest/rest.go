...
import (
	...
	"github.com/cheeseandcereal/aggexp/experiments/0049-locked-write-transaction/pkg/backend"
	"github.com/cheeseandcereal/aggexp/experiments/0049-locked-write-transaction/pkg/locking"
	...
	perwatch "github.com/cheeseandcereal/aggexp/experiments/0049-locked-write-transaction/pkg/watch"
)
...
func (r *REST) Create(ctx context.Context, obj runtime.Object, createValidation rest.ValidateObjectFunc, _ *metav1.CreateOptions) (runtime.Object, error) {
	...
	// 0049: acquire -> body -> commit run as ONE retriable transaction
	// (README candidate (a)). On Create the metadata CR does not exist,
	// so the first Acquire CAS-creates it carrying only the lock; a CAS
	// conflict at body/commit re-reads and retries within the budget,
	// and budget exhaustion surfaces a clean 409 (never a 500).
	var committedBody backend.Body
	res, terr := r.locker.WriteTxn(ctx, ref, locking.WriteOp{
		WriteBody: func(ctx context.Context) error {
			body := backend.BodyFromWidget(w)
			body.Owner = owner
			committedBody = body
			return r.bodies.Put(ctx, w.Namespace, w.Name, body)
		},
		BuildRecord: func(ctx context.Context, _ int) (*metastore.Record, error) {
			...
			rec.BodyHash = backend.HashBody(committedBody)
			return rec, nil
		},
		OnConflictRetry: func(attempt int) {
			r.counters.WriteRetry.Add(1)
			klog.V(2).InfoS("create-txn-retry", "replica", r.replicaID, "ref", refLog(ref), "attempt", attempt)
		},
	}, nil)
	if terr != nil {
		// On a hard failure with no record committed, clean up any body
		// we wrote so a retry by the client is clean.
	...
	stitched := r.stitch(w.Namespace, w.Name, committedBody, res.Record)
	return stitched, nil
}
...
func (r *REST) Update(
	...
	// 0049: acquire -> body -> commit as ONE retriable transaction.
	// The 0043 PRE-ACQUIRE OCC check runs once per attempt against the
	// served object's CURRENT authoritative RV (captured before that
	// attempt's acquire bumps it). A genuine OCC failure (a peer
	// committed a different RV) is a legitimate 409 and is not retried;
	// a bare CAS conflict on the body/commit writes IS retried within
	// the budget, and budget exhaustion surfaces a clean 409.
	var committedBody backend.Body
	res, terr := r.locker.WriteTxn(ctx, ref, locking.WriteOp{
		WriteBody: func(ctx context.Context) error {
			body := backend.BodyFromWidget(w)
			body.Owner = owner
			committedBody = body
			return r.bodies.Put(ctx, w.Namespace, name, body)
		},
		BuildRecord: func(ctx context.Context, _ int) (*metastore.Record, error) {
			rec := recordFromObject(w, ref)
		...
		OnConflictRetry: func(attempt int) {
			r.counters.WriteRetry.Add(1)
			klog.V(2).InfoS("update-txn-retry", "replica", r.replicaID, "ref", refLog(ref), "attempt", attempt)
		},
	}, func(curRV string) error {
		if clientRV != "" && curRV != "" && clientRV != curRV {
			return apierrors.NewConflict(groupResource, name,
	...
	stitched := r.stitch(w.Namespace, name, committedBody, res.Record)
...
// recordWriteOutcome records a successful served write and tracks the
// high-water transaction retry depth (0049 instrumentation).
