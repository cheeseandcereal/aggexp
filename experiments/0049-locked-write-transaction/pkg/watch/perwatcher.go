...
type Hub struct {
	...
	// path (0048). A single metadata CR carries both the watcher-
	// visible state (body hash + KRM metadata) AND the embedded lock.
	// Lock acquire/release/renewal write the CR and fire a MODIFIED
	// informer event, but change nothing a watcher should see. Keyed by
	// metadata-CR record name → last emitted VisibleSignature; a
	// MODIFIED whose signature is unchanged is suppressed BEFORE the
	// per-watcher fan-out, so lock churn never reaches any watcher.
	// 0047 re-homing point: the filter lives here (keyed on
	// VisibleSignature) rather than the single broadcaster, because the
	// per-watcher path is what does the fan-out now.
...
//
// 0048 RE-HOMED EMISSION FILTER (from 0043): the metadata CR doubles
// as the embedded lock. Lock acquire/release/renewal fire a MODIFIED
// with no watcher-visible change. Before fanning out, compute the
// record's VisibleSignature and suppress a MODIFIED whose signature is
// unchanged from the last emission for this record. Without this, one
// served write surfaces as up to three MODIFIEDs (acquire/body/release)
// plus a renewal drip on slow ops — to EVERY watcher.
func (h *Hub) OnMetadataEvent(et watch.EventType, ref metastore.ResourceRef, rec *metastore.Record, rv string) {
	...
	case watch.Modified:
		...
		if seen && prev == sig {
			h.mu.Unlock()
			klog.V(2).InfoS("emission-filter-suppress", "ref", refKey(ref), "rv", rv, "reason", "lock-or-renewal-only")
			return
		}
		h.lastSig[name] = sig
		h.mu.Unlock()
	case watch.Added:
		h.mu.Lock()
		h.lastSig[name] = metastore.VisibleSignature(rec)
		h.mu.Unlock()
	case watch.Deleted:
		h.mu.Lock()
...
func refKey(r metastore.ResourceRef) string {
	ns := r.Namespace
	if ns == "" {
		ns = "cluster"
	}
	return r.Group + "/" + r.Resource + "/" + ns + "/" + r.Name
}
