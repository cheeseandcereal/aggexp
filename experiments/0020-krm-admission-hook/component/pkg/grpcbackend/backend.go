...
import (
	...
	"github.com/cheeseandcereal/aggexp/experiments/0020-krm-admission-hook/component/pkg/dyn"
)
...
// admit runs the 0020 admission pipeline: Mutate (if supported),
// then Validate (if supported). Returns the possibly-mutated object
// JSON, or an error if either hook denied the request.
//
...
//
// Mutating before validating is the same ordering Kubernetes uses
// for admission webhooks: mutating webhooks run first so their
// changes are visible to the validating webhooks that follow.
