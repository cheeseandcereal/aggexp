// Command note-backend is the reference backend for experiment
// 0021. It serves a single resource type — Note — via the
// runtime/component/proto Backend gRPC service. It deliberately
// does not import k8s.io/apiserver; the point is that a backend
// can expose a Kubernetes API through the component substrate
// without pulling in any Kubernetes machinery.
package main
...
// Note is the resource this backend serves.
type Note struct {
...
func (b *backend) GetSchema(_ context.Context, _ *componentpb.GetSchemaRequest) (*componentpb.GetSchemaResponse, error) {
	schema := map[string]any{
		"type":        "object",
		"description": "Note is a free-form piece of text served by the 0021 runtime-component-parity experiment.",
			...
			"spec": map[string]any{
				...
				"properties": map[string]any{
					"title": map[string]any{"type": "string",
						"description": "Short display title; rendered by kubectl get."},
					"body": map[string]any{"type": "string",
						"description": "Free-form body text."},
				},
			},
...
func (b *backend) Apply(_ context.Context, req *componentpb.ApplyRequest) (*componentpb.ApplyResponse, error) {
	var n Note
	if err := json.Unmarshal(req.ObjectJson, &n); err != nil {
		return nil, status.Errorf(codes.InvalidArgument, "unmarshal object: %v", err)
	}
	...
	raw, _ := json.Marshal(&n)
	evType := componentpb.EventType_EVENT_MODIFIED
	if created {
		evType = componentpb.EventType_EVENT_ADDED
	}
	...
	return &componentpb.ApplyResponse{ObjectJson: raw, Created: created}, nil
}
