...
for i in $(seq 1 20); do
  status=$(kubectl get apiservice v1.aggexp.io -o jsonpath='{.status.conditions[?(@.type=="Available")].status}')
  if [[ "${status}" == "True" ]]; then
...
echo "warning: APIService did not return to Available within 20s after pinning to ${svc}"
exit 1
