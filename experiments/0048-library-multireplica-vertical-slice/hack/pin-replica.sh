...
#
# Requires the kind-aggexp-0048 context to be current.
...
kubectl patch apiservice v1.widgets.aggexp.io --type=merge \
  -p "{\"spec\":{\"service\":{\"namespace\":\"aggexp-system\",\"name\":\"${svc}\",\"port\":443}}}"

for i in $(seq 1 30); do
  status=$(kubectl get apiservice v1.widgets.aggexp.io -o jsonpath='{.status.conditions[?(@.type=="Available")].status}' 2>/dev/null || true)
  if [[ "${status}" == "True" ]]; then
